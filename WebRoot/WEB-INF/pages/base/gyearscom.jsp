		...
		$('#downOfExcel')
				.click(
						function() {
							...
							//首先 ajax 判断该年级下的班级有没有完成班级测评
							$.post('${pageContext.request.contextPath}/student_isCompleteScom.action', {
					    				termid: termid,
	                                	yid:yid
	                                }, function (data) {
	                                	// 完成 1 ,0 未完成
	                                	if(data=='0'){
	                                		$.messager.alert('未完成','还未完成班级测评 ,不能进行综合测评!','info');  
	                                	}else{
	                                		window.location.href = "${pageContext.request.contextPath}/student_getScomYear.action?yid="
	        									+ yid + "&termid=" + termid;
	                                	}
                                	});	
						});
